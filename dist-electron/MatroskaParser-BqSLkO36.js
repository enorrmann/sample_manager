import { E as w, c as f, b as S, q as h, r as E, S as T, m as I, t as $, B as z, u as D, T as N } from "./handlers-Krk-dW_N.js";
...
class U {
  ...
  async iterate(a, t, u) {
    return this.parseContainer(k(a), t, u);
  }
  async parseContainer(a, t, u) {
    const s = {};
    for (; this.tokenizer.position < t; ) {
      ...
      if (n)
        switch (u.startNext(n)) {
          case c.ReadNext:
            if (r.id, g(`Read element: name=${x(n)}{id=0x${r.id.toString(16)}, container=${!!n.container}} at position=${o}`), n.container) {
              const m = await this.parseContainer(n, r.len >= 0 ? this.tokenizer.position + r.len : -1, u);
              n.multiple ? (s[n.name] || (s[n.name] = []), s[n.name].push(m)) : s[n.name] = m, await u.elementValue(n, m, o);
            } else {
              const m = this.parserMap.get(n.value);
              if (typeof m == "function") {
                const d = await m(r);
                s[n.name] = d, await u.elementValue(n, d, o);
              }
            }
            break;
          case c.SkipElement:
...
function M(l, a) {
  const t = new Uint8Array(8), u = l.subarray(0, a);
  try {
    return t.set(u, 8 - a), $.get(t, 0);
  } catch {
    return BigInt(-1);
  }
}
function k(l) {
  return l.container && Object.keys(l.container).map((a) => {
    const t = l.container[a];
...
function x(l) {
  let a = "";
  return l.parent && l.parent.name !== "dtd" && (a += `${x(l.parent)}/`), a + l.name;
}
  ...
  async parse() {
    ...
    p("Initializing DTD end MatroskaIterator"), await t.iterate(F, a, {
      startNext: (u) => {
        switch (u.id) {
          case 475249515:
            return p(`Skip element: name=${u.name}, id=0x${u.id.toString(16)}`), c.IgnoreElement;
          case 524531317:
      ...
      elementValue: async (u, s, r) => {
        switch (p(`Received: name=${u.name}, value=${s}`), u.id) {
          case 17026:
          ...
          case 374648427:
            {
              ...
              if (o != null && o.entries) {
                ...
                const n = o.entries.filter((i) => i.trackType === N.audio).reduce((i, m) => !i || m.flagDefault && !i.flagDefault || m.trackNumber < i.trackNumber ? m : i, null);
          ...
          case 307544935:
            {
              ...
              await Promise.all(o.tag.map(async (n) => {
                const i = n.target, m = i != null && i.targetTypeValue ? D[i.targetTypeValue] : i != null && i.targetType ? i.targetType : "track";
